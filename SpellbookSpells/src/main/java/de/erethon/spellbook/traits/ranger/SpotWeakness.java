...
public class SpotWeakness extends SpellTrait {

    private final int duration = data.getInt("duration", 100);
    private final int stacks = data.getInt("stacks", 1);

    public SpotWeakness(TraitData data, LivingEntity caster) {
        super(data, caster);
    }
    ...
    protected void onTrigger(TraitTrigger trigger) {
        for (LivingEntity living : trigger.getTargets()) {
            living.addEffect(caster, Spellbook.getEffectData("Weakness"), duration, stacks);
        }
    }
}
